...
export default function App() {
  ...
  useOnClickOutside(popupRef, () => {setPopupState(false);setLoginPopupMethod(null);setLoginPopupStep(0)});
  if (!session) {
    return (
      <Layout>
        ...
        <Popup popupState={popupState} ref={popupRef} center id={styles["login-popup"]}>
          {loginPopupStep==0?(<LoginChooser setPopupState={setPopupState} setStep={setLoginPopupStep} method={loginPopupMethod} setMethod={setLoginPopupMethod}/>):<LoginForm method={loginPopupMethod} setOpen={setPopupState}/>}
        </Popup>
      </Layout>
    );
  }
